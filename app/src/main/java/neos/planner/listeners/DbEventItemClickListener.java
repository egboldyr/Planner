 ...
 * Project: Planner
 *
 * Cлушатель события нажатие на один из элементов списка событий
 */
...
public class DbEventItemClickListener implements RecyclerView.OnItemTouchListener {
    ...
    public DbEventItemClickListener(Context context, OnItemClickListener listener) {
        ...
        if (detector.onTouchEvent(e)) {
            ...
            if (view != null && !view.dispatchTouchEvent(e)) {
                if (rv.getChildPosition(view) == RecyclerView.NO_POSITION) {
                    //Возвращаем true так как нажата дочерняя клавиша ShareButton или RemoveButton
                    return true;
                }
